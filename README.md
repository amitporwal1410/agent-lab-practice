...
1. A `tools.ts`-style file defines what the agent can *do*: each tool has a JSON schema
   (name, description, inputs) the model reads, and a `runTool` function that actually
   executes it (fake in-memory data, no real APIs, is fine for practice).
2. An `agent.ts`-style file runs the loop:
...
- **Build a new scenario live.** Write a tools file (schema + `runTool` dispatch) and
  an agent file (the loop above) for whatever domain they hand you — the loop itself
